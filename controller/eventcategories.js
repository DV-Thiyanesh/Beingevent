...
module.exports = function (app) {
 ...
 app.get("/eventcategories/:type/:page", async function (req, res) {
 ...
 let currentDate= new Date()
        ...
        //  events.find({eventStartDate:{$lt:currentDate}}).sort({eventStartDate: 1}).exec(function (err, events) {
            ...
            try {
                var firstQueryResult = await events.find({"eventType": type,eventStartDate:{$gte:currentDate}}).sort({eventStartDate: 1}).exec()
                  length=events.length;
                   
                    var secondQueryResult = await events.find({"eventType": type,eventStartDate:{$gte:currentDate}}).sort({eventStartDate: 1}) .skip((perpage * page) - perpage)
